describe('Touch assertions', function () {
  ...
  it('Zoom in and out with pinch', function () {
    ...
    cy.get('img').should('have.css', 'transform', 'matrix(2, 0, 0, 2, 500, 261.56)')
    cy.get('[data-testid=container]').pinch({ distance: -4 })
    cy.get('img').should('have.css', 'transform', 'matrix(1.2, 0, 0, 1.2, 100, 36.9361)')
  })
  ...
  it('Zoom in and out with pinch based on the center between 2 fingers', function () {
    cy.get('[data-testid=container]')
      ...
      .trigger('touchend')
    cy.get('img').should('have.css', 'transform', 'matrix(1.1, 0, 0, 1.1, 0, 4.25812)')
    cy.get('[data-testid=container]')
      .trigger('touchstart', {
        touches: [
      ...
      .trigger('touchend')
    cy.get('img').should('have.css', 'transform', 'matrix(1, 0, 0, 1, -36.8182, -0.219895)')
  })
  ...
  it('Move image with pinch based on the center between 2 fingers', function () {
    cy.get('[data-testid=container]')
      ...
      .trigger('touchend')
    cy.get('img').should('have.css', 'transform', 'matrix(1, 0, 0, 1, 100, -0.219895)')
  })
  ...
  it('Rotate with pinch', function () {
    ...
    cy.get('img').should(
      'have.css',
      'transform',
      'matrix(0.866025, 0.5, -0.5, 0.866025, -2.5, -0.219895)'
    )
    cy.get('[data-testid=container]').rotate({ rotation: -45 })
    cy.get('img').should(
      'have.css',
      'transform',
      'matrix(0.965926, -0.258819, 0.258819, 0.965926, 1.03553, -1.68436)'
    )
  })
})
