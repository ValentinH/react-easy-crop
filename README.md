...
| `objectFit` [demo](https://codesandbox.io/s/react-easy-crop-forked-p9r34) | 'contain', 'horizontal-cover', 'vertical-cover', 'auto-cover' or 'fit'      |          | Specifies how the image is shown in the cropper:. `contain`: the image will be adjusted to be fully visible, `horizontal-cover`: the image will horizontally fill the cropper, `vertical-cover`: the image will vertically fill the cropper, `auto-cover`: we automatically pick between `horizontal-cover` or `vertical-cover` to have fully visible image inside the copper area, `fit`: in oposite to `auto-cover`, scales up the image to fullfill the cropped area. Defaults to "contain".               |
